...
from models import AlexNet, DenseNet, GoogLeNet, LeNet, ResNet, VGG, VisionTransformer, PointFusion, GLIP, \
    DecisionFusion, WFCG, MVC
...
def _freeze_params(model: nn.Module, args: Args) -> None:
    match args.model_name:  # 低层次特征冻结，首次卷积出最大通道或者足够大通道开始视作高层次特征（拼接不算）
        ...
        case "vit_b_16" | "decision_fusion":  # vit 和上述不同，这里选择冻掉 patch_embed 和 encoder 至少 20% 的层数
        ...
        case "lenet5":
            pass
        case _:  # other fusion (vit + densenet)
            layer_names = ["encoder_layer_" + str(i) for i in range(3)]
            model.vit_b_16.conv_proj.requires_grad_(False)
            for attr in layer_names:
                getattr(model.vit_b_16.encoder.layers, attr).requires_grad_(False)
...
def _init_params(model: nn.Module, args: Args) -> None:
    ...
    else:  # lenet5 & fusion (vit or vit + densenet)
        for child in model.children():
            if type(child) == torchvision.models.VisionTransformer or type(child) == torchvision.models.DenseNet:
                continue  # fusion (vit + densenet)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_name", default="point_fusion", type=str)
    parser.add_argument("--dataset_name", default="animals_10_aug", type=str)
    ...
    if args.pretrained_path is None:
        args.pretrained_path = pretrained_paths["vit_b_16"]
        args.pretrained_path2 = pretrained_paths["densenet201"]

    model = args.model_class(args)
    data_dir = data_dirs[args.dataset_name]
    ...
    _save_log(model, dataset, trainer, args, parser_args, True)
